model = dict(
    ...
    bbox_head=dict(
        ...
        limit=27,
        centerness_topk=18),
    voxel_size=(.16, .16, .16), # width, depth, height
    n_voxels=(40, 40, 16),
    volume_type="cov_w_mean", # mean, cov, ... more tbd.
    pc_supervise=True
    )
train_cfg = dict()
test_cfg = dict(
...
input_modality = dict(use_image=True, use_depth=False, use_lidar=True, use_neuralrecon_depth=False, use_ray=False)
...
if input_modality['use_lidar']:
    train_collect_keys.append('points')
    test_collect_keys.append('points')
